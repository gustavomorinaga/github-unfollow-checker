...
/**
 * A custom hook of `@uidotdev/usehooks` to use the `localStorage` API.
 *
 * The original hook doesn't handle server-side rendering, so I picked the
 * solution from the issue: https://github.com/uidotdev/usehooks/issues/318#issuecomment-2471580885
 */
export function useLocalStorage<T>(
	...
	useEffect(() => {
		...
		function handleStorageChange(event: StorageEvent) {
			if (event.key === key && event.newValue) setLocalState(JSON.parse(event.newValue));
		}
